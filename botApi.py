...
bot = SimpleLongPollBot(tokens=token,
                        group_id=199735512)


@bot.message_handler(bot.text_contains_filter("пивобот лучшее пиво"))
async def handle_best_pivo(event: bot.SimpleBotEvent) -> str:
    await event.answer(message="Кончено же ипа!")


@bot.message_handler(bot.text_contains_filter("пивобот случайное пиво"))
async def random_beer(event: bot.SimpleBotEvent):
    beer = randomBeerParser()
    msg = "Почему бы сегодня не выпить: \n"
    msg += "🍺" + beer['name'] + '\n' + "⭐" + beer['rating'] + "\n 💬Отзывы:\n" + beer['url']
    image_url = beer['image']

    attach = await uploader.get_attachments_from_links(
        peer_id=event.peer_id,
        links=[beer['image']]
    )
    await event.answer(message=msg, attachment=attach)
    # session = requests.Session()
    # upload = vk_api.VkUpload(vk_session)
    # image = session.get(image_url, stream=True)
    # photo = upload.photo_messages(photos=image.raw)[0]
